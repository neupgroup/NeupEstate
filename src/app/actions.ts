...
import { createTeamMember as createTeamMemberService, updateTeamMember as updateTeamMemberService, deleteTeamMember as deleteTeamMemberService } from '@/services/team-service';
...
import type { NaturalLanguageSearchOutput, Property, CreatePropertyInput, UpdatePropertyInput, CreateAgencyInput, UpdateAgencyInput, PropertyApprovalResult, CreatePropertyFormValues, UpdatePropertyFormValues, CreateAgencyFormValues, UpdateAgencyFormValues, PropertyFilters, ExtractedPropertyData, SitemapLog, PropertyAmendmentResult, RewritePropertyDetailsOutput, PropertyAssuranceResult, Agent, CreateAgentFormValues, UpdateAgentFormValues, StructuredLocation, CreateWhatsAppTemplateFormValues, WhatsAppConfig, WhatsAppTemplate, CreateConversationFormValues, CreateUserActivityInput, PropertyImageUpdateResult, CreateTeamMemberFormValues, UpdateTeamMemberFormValues, UpdateAboutPageContentFormValues, CreateFaqFormValues, UpdateFaqFormValues, CreateInquiryFormValues, InquiryStatus, UpdatePromptFormValues, CreatePromptFormValues, User, CreatePropertyRequestFormValues, CreateSalesRequestFormValues, CreateVisitRequestFormValues, CreateMortgageRequestFormValues, CreateContactSubmissionFormValues, CreateBlogFormValues, UpdateBlogFormValues, CreateNewsletterSubscriptionFormValues, PropertyActivityEvent, UserPreferences, AIModel, CreateAIModelFormValues, UpdateAIModelFormValues, CreateRequirementFormValues, Requirement, UpdateUserFormValues } from "@/types";
import { CreatePropertySchema, UpdatePropertySchema, CreateAgencySchema, UpdateAgencySchema, PropertyPurposeSchema, PropertyCategorySchema, PropertyUsageTypeSchema, CreateAgentSchema, UpdateAgentSchema, CreateWhatsAppTemplateSchema, WhatsAppConfigSchema, CreateConversationSchema, CreateTeamMemberSchema, UpdateTeamMemberSchema, UpdateAboutPageContentSchema, CreateFaqSchema, UpdateFaqSchema, CreateInquirySchema, UpdatePromptSchema, CreatePromptSchema, CreatePropertyRequestSchema, CreateSalesRequestSchema, CreateVisitRequestSchema, CreateMortgageRequestSchema, CreateContactSubmissionSchema, CreateBlogSchema, UpdateBlogSchema, CreateNewsletterSubscriptionSchema, CreateAIModelSchema, UpdateAIModelSchema, CreateRequirementSchema, UpdateUserSchema } from "@/types";
...
import { updateAboutPageContent as updateAboutPageContentService } from '@/services/site-content-service';
...
import { createBlog as createBlogService, updateBlog as updateBlogService, deleteBlog as deleteBlogService } from '@/services/blog-service';
import { createNewsletterSubscription as createNewsletterSubscriptionService } from '@/services/newsletter-service';
...
export async function createTeamMemberAction(
  data: CreateTeamMemberFormValues
): Promise<{ success: boolean; error?: string | null; memberId?: string | null }> {
  try {
    const validatedData = CreateTeamMemberSchema.parse(data);
    const memberId = await createTeamMemberService(validatedData);
    revalidatePath('/manage/team');
    revalidatePath('/team');
    return { success: true, memberId, error: null };
  } catch (e: any) {
    await logProblem(e, 'createTeamMemberAction');
    if (e instanceof z.ZodError) {
        return { success: false, error: e.message, memberId: null };
    }
    return { success: false, error: "An unexpected server error occurred.", memberId: null };
  }
}

export async function updateTeamMemberAction(
  id: string,
  data: UpdateTeamMemberFormValues
): Promise<{ success: boolean; error?: string | null; }> {
  try {
    const validatedData = UpdateTeamMemberSchema.parse(data);
    await updateTeamMemberService(id, validatedData);
    revalidatePath('/manage/team');
    revalidatePath('/team');
    revalidatePath(`/manage/team/${id}/edit`);
    return { success: true, error: null };
  } catch (e: any) {
    await logProblem(e, `updateTeamMemberAction (ID: ${id})`);
    if (e instanceof z.ZodError) {
        return { success: false, error: e.message };
    }
    return { success: false, error: "An unexpected server error occurred." };
...
export async function deleteTeamMemberAction(memberId: string) {
    try {
        ...
        await logProblem(error, `deleteTeamMemberAction (ID: ${memberId})`);
...
export async function updateAboutPageContentAction(
  data: UpdateAboutPageContentFormValues
): Promise<{ success: boolean; error?: string | null; }> {
  try {
    const validatedData = UpdateAboutPageContentSchema.parse(data);
    await updateAboutPageContentService(validatedData);
    revalidatePath('/manage/about');
    revalidatePath('/about'); // Revalidate the public page as well
    return { success: true, error: null };
  } catch (e: any) {
    await logProblem(e, `updateAboutPageContentAction`);
    if (e instanceof z.ZodError) {
        return { success: false, error: e.message };
    }
    return { success: false, error: "An unexpected server error occurred." };
...
// Blog Actions
export async function createBlogAction(data: CreateBlogFormValues): Promise<{ success: boolean; error?: string }> {
    try {
        const validatedData = CreateBlogSchema.parse(data);
        await createBlogService(validatedData);
        ...
        await logProblem(e, 'createBlogAction');
        return { success: false, error: e.message || "Failed to create blog post." };
    }
}

export async function updateBlogAction(data: UpdateBlogFormValues): Promise<{ success: boolean; error?: string }> {
    try {
        const validatedData = UpdateBlogSchema.parse(data);
        await updateBlogService(validatedData.id, validatedData);
        revalidatePath('/manage/blogs');
        revalidatePath(`/blogs/${validatedData.id}`);
        return { success: true };
    } catch (e: any) {
        if (e instanceof z.ZodError) {
            return { success: false, error: e.message };
        }
        await logProblem(e, `updateBlogAction (ID: ${data.id})`);
...
export async function deleteBlogAction(blogId: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        await logProblem(e, `deleteBlogAction (ID: ${blogId})`);
        return { success: false, error: e.message || "Failed to delete blog post." };
    }
}

// Newsletter Action
export async function createNewsletterSubscriptionAction(data: CreateNewsletterSubscriptionFormValues): Promise<{ success: boolean; error?: string }> {
    try {
        const validatedData = CreateNewsletterSubscriptionSchema.parse(data);
        await createNewsletterSubscriptionService(validatedData);
        ...
        await logProblem(e, 'createNewsletterSubscriptionAction');
