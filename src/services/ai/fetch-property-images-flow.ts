...
const imageExtractionPrompt = ai.definePrompt({
    name: 'imageExtractionPrompt',
    input: { schema: z.object({ sourceUrl: z.string().url() }) },
    output: { schema: z.object({
        images: z.array(z.string().url()).describe("A list of absolute URLs for images that are clearly part of the main property gallery or listing."),
    }) },
    tools: [fetchPageContent],
    prompt: `You are an AI data scraper specializing in extracting property images from a live webpage. Your task is to analyze the page's HTML and return a list of absolute URLs for images that are clearly part of the main property gallery or listing.
...
const fetchPropertyImagesFlow = ai.defineFlow(
  {
    name: 'fetchPropertyImagesFlow',
    inputSchema: FetchPropertyImagesInputSchema,
    outputSchema: FetchPropertyImagesOutputSchema,
  },
  async ({ url }) => {
    try {
        const { output } = await imageExtractionPrompt({ sourceUrl: url });
